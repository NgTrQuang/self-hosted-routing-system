...
import { MapPin, Navigation, Table2, Activity, AlertCircle, Loader2, ChevronDown, ChevronUp, Truck, Plus, Trash2, RotateCcw } from 'lucide-react'
...
function StatusBadge({ health }) {
  if (health === null) return <span className="text-xs text-gray-400 flex items-center gap-1"><Loader2 size={12} className="animate-spin" />Checking...</span>
  if (health === 'ok') return <span className="text-xs text-emerald-400 flex items-center gap-1"><Activity size={12} />API Online</span>
  return <span className="text-xs text-red-400 flex items-center gap-1"><AlertCircle size={12} />API Offline</span>
}
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <header className="bg-gray-900 border-b border-gray-800 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-white leading-tight">Self-Hosted Routing System</h1>
            <p className="text-xs text-gray-400">Vietnam · OSRM + OpenStreetMap</p>
          </div>
        </div>
          ...
          <div className="flex-1 p-4 flex flex-col gap-4">
            {tab === 'route' && (
              <>
                {/* Presets */}
                <div>
                  <p className="text-xs text-gray-500 mb-2 uppercase tracking-wider">Preset Routes</p>
                ...
                {/* Map pick mode */}
                <div>
                  <p className="text-xs text-gray-500 mb-2 uppercase tracking-wider">Click on map to pick</p>
                  ...
                  {pickMode && (
                    <p className="text-xs text-yellow-400 mt-1.5">
                      Click on the map to set {pickMode === 'origin' ? 'origin (green)' : 'destination (red)'}
                    </p>
                  )}
                </div>
                ...
                <form onSubmit={handleRoute} className="flex flex-col gap-3">
                  <div className="bg-gray-800 rounded-lg p-3 flex flex-col gap-2">
                    <SearchBox label="Origin" color="green" onSelect={handleOriginSelect} />
                  ...
                  <div className="bg-gray-800 rounded-lg p-3 flex flex-col gap-2">
                    <SearchBox label="Destination" color="red" onSelect={handleDestSelect} />
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <div>
                        <label className="text-xs text-gray-500">Latitude</label>
                  ...
                  <button type="submit" disabled={routeLoading}
                    ...
                    {routeLoading ? <><Loader2 size={15} className="animate-spin" />Calculating...</> : <><Navigation size={15} />Get Route</>}
                  </button>
                </form>
                ...
                {routeResult && (
                  <div className="bg-blue-900/30 border border-blue-700/50 rounded-lg p-4 flex flex-col gap-3">
                    <p className="text-xs text-blue-400 uppercase tracking-wider">Route Result</p>
                    ...
                    <p className="text-xs text-gray-500 text-center">
                      Avg speed: {((routeResult.distanceMeters / routeResult.durationSeconds) * 3.6).toFixed(0)} km/h
                      {routeResult.geometry?.length > 0 && <span className="ml-2 text-emerald-500">· Đường thực ✓</span>}
                    </p>
                    {routeResult.steps?.length > 0 && (
                      <div>
                        <button onClick={() => setShowSteps(v => !v)}
                          ...
                          Chỉ đường ({routeResult.steps.length} bước)
                        </button>
                        {showSteps && (
              ...
              <>
                <div className="bg-gray-800/60 rounded-lg p-3">
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Nhập các điểm giao hàng — hệ thống tự tính <span className="text-yellow-400 font-medium">thứ tự tối ưu</span> để tổng quãng đường ngắn nhất.
                  </p>
                </div>
                <form onSubmit={handleTrip} className="flex flex-col gap-3">
                  <div className="bg-gray-800 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs text-emerald-400 font-medium">Điểm dừng <span className="text-gray-500">(lat,lng mỗi dòng, điểm đầu = kho)</span></p>
                    </div>
                    <textarea
                      value={tripStops}
                  ...
                  <button type="submit" disabled={tripLoading}
                    className="bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 text-white text-sm font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors">
                    {tripLoading ? <><Loader2 size={15} className="animate-spin" />Đang tối ưu...</> : <><Truck size={15} />Tối ưu tuyến giao hàng</>}
                  </button>
                </form>
                ...
                {tripResult && (
                  <div className="bg-blue-900/30 border border-blue-700/50 rounded-lg p-4 flex flex-col gap-3">
                    <p className="text-xs text-blue-400 uppercase tracking-wider">Kết quả tối ưu</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-800/60 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-white">{formatDistance(tripResult.distanceMeters)}</p>
                        <p className="text-xs text-gray-400 mt-1">Tổng quãng đường</p>
                      </div>
                      <div className="bg-gray-800/60 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-white">{formatDuration(tripResult.durationSeconds)}</p>
                        <p className="text-xs text-gray-400 mt-1">Tổng thời gian</p>
                      </div>
                    </div>
                    <div className="bg-gray-800/60 rounded-lg p-3">
                      <p className="text-xs text-yellow-400 font-medium mb-2">Thứ tự tối ưu</p>
                        ...
                        {(() => {
                          ...
                          return tripResult.waypointOrder.map((origIdx, visitOrder) => (
                            <div key={visitOrder} className="flex items-center gap-2 text-xs">
                              ...
                              {visitOrder === 0 && <span className="text-emerald-400 text-[10px]">← Kho</span>}
                            </div>
                          ))
                        })()}
                      </div>
                    ...
                    <div className="flex flex-col gap-1">
                      {tripResult.legs.map((leg, i) => (
                        <div key={i} className="flex items-center justify-between text-xs bg-gray-800/40 rounded px-2 py-1.5">
                          <span className="text-gray-400">Chặng {i + 1}</span>
            ...
            {tab === 'matrix' && (
              <>
                <form onSubmit={handleMatrix} className="flex flex-col gap-3">
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-emerald-400 font-medium mb-2">Origins <span className="text-gray-500">(lat,lng per line)</span></p>
                    <textarea
                      value={matrixOrigins}
                  ...
                  <div className="bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-red-400 font-medium mb-2">Destinations <span className="text-gray-500">(lat,lng per line)</span></p>
                    <textarea
                      value={matrixDests}
                  ...
                  <button type="submit" disabled={matrixLoading}
                    className="bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 text-white text-sm font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors">
                    {matrixLoading ? <><Loader2 size={15} className="animate-spin" />Computing...</> : <><Table2 size={15} />Compute Matrix</>}
                  </button>
                </form>
                ...
                {matrixResult && (
                  <div className="bg-gray-800/60 rounded-lg p-3">
                    <p className="text-xs text-blue-400 uppercase tracking-wider mb-3">Distance Matrix (km)</p>
                    ...
                    <p className="text-xs text-blue-400 uppercase tracking-wider mt-3 mb-3">Duration Matrix (min)</p>
                    <div className="overflow-x-auto">
                      <table className="text-xs w-full">
                        <thead>
                          <tr>
        ...
        <main className="flex-1 relative">
          {pickMode && (
            <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[1000] bg-gray-900/95 border border-yellow-500/50 rounded-full px-4 py-1.5 text-xs text-yellow-300 pointer-events-none shadow-lg">
              Click on map to set {pickMode === 'origin' ? '🟢 origin' : '🔴 destination'} — or click sidebar button to cancel
            </div>
          )}
          <MapContainer
